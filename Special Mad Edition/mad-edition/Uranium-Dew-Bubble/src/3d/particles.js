...
function init(renderer, camera) {
    ...
    _addSphereTexture('default', quickLoader.add('images/matcap.jpg', {
        onLoad: _onSphereMapLoad.bind('default')
    }).content);
    _addSphereTexture('metal', settings.sphereMap);
    _addSphereTexture('plastic', quickLoader.add('images/logo.png', {
        onLoad: _onSphereMapLoad.bind('plastic')
    }).content);
    ...
    _particlesMaterial = new THREE.ShaderMaterial({
        uniforms: {
            ...
            uAdditive: {
                type: 't',
                value: _additiveRenderTarget
            },
            uSphereMap: {
                type: 't',
                value: sphereTextures.default
            },
            uResolution: {
                type: 'v2',
                value: _resolution
